...
contains
  ...
  subroutine Init_SEoptions(O_SEopt,I_NameListFile)
    ...
    character(len=MAX_STRING_LEN):: BaseName           = 'NULL'
    character(len=MAX_FILE_LEN)  :: InputPath          = './'
    character(len=MAX_FILE_LEN)  :: OutputPath         = './'
    ...
    MeshFile        = trim(GridName)

    ! Find the position of '.g' in GridName
    pos = index(MeshFile, '.g')
    ! If '.g' is found, remove it
    if (pos /= 0) then
      BaseName = MeshFile(1:pos-1)
    else
      BaseName = MeshFile
    endif

    SCRIP_filename  = trim(BaseName)//'_np4_SCRIP.nc'
    LATLON_filename = trim(BaseName)//'_np4_LATLON.nc'
